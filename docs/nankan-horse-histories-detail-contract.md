...
- ただしこれは entries 運用を止める理由ではなく、**運用上の注意**として整理する。
  自動取得・大量取得をしない手作業コピペ運用であれば小さく再開できる。
...
- **判断**: **robots.txt 不在を「許可」と扱わない**。**uma_info の自動取得・大量取得はしない**方針を維持する。
  外部サイト由来データの扱いには注意し、利用条件・取得負荷（礼儀的アクセス間隔）は必要に応じて確認する。
  entries の手作業コピペ運用（自動取得を伴わない）は、この直接取得とは別物として小さく再開できる。
...
PR-D2（dry-run 設計）の**取得部分**に進むには、次を**すべて**満たす必要がある:
1. **nankankeiba.com `uma_info`（馬単体ページ）の規約・許諾・取得負荷・アクセス間隔のクリアランス**
   （著作権 "all rights reserved" 明示・robots.txt 不在を許可扱いしない。利用規約本文の精査と許諾判断・礼儀的 delay の設計）。
...
- 現状はいずれも**未充足**。C（keiba.go.jp `HorseMarkInfo`）は robots Disallow で**不可**、D（自前集約）は母数不足で正本不可。
...
- **ただし規約面の前提（厳守）**:
  - **robots.txt 不在（404）を「許可」と扱わない**。
  - 著作権 "copyright(C)nankankeiba.com all rights reserved." が明示。
  - **利用規約・著作権・取得負荷・アクセス間隔（礼儀的 delay）の確認が必要**。クリアするまで取得しない。
...
### 12.10.1 結論：利用条件は未確定（現時点では採用未決定）

- **`uma_info` は全履歴 `history[]` の技術的な本命候補**である（§12.9.3：デビューからの全出走履歴＋通算＋条件別を 1 ページで持つ）。
- **ただし利用規約精査の結果、利用条件は未確定であり、現時点では採用未決定とする。**
  **uma_info の自動取得・大量取得はしない**方針を維持し、利用条件・運用方針が固まるまで本格展開しない。
...
- 規約本文に **「別に許可を頂いている場合を除き」＋「個人的・家庭的利用に限定」** の趣旨の条項あり
  → **利用範囲の解釈には確認余地がある**（複製・転載・転用・二次利用・再配布・データベース化の扱いは
  必要に応じて確認する）。
- ⚠️ **規約本文の取得に文字化け・解釈余地があり、可否の語尾を断定できない箇所がある**。
  **明示の許可とは判断しない**（外部サイト由来データの扱いには注意する）。利用条件は未確定。
...
- 外部サイト由来データの扱いには注意する。**自動取得・大量取得はしない**方針を前提に、外部データ利用方針を確認する。
...
- → **利用条件・運用方針が固まるまで、`uma_info` を取得元にした全履歴 `history[]` 生成は本格展開しない**（現状ほかに現実的な全履歴取得元が無い）。

### 12.10.9 PR-D2 進行条件（更新・運用方針が固まるまで保留）

**PR-D2 は運用方針が固まるまで保留**。取得部分に進むには次を**確認・整理**する:
...
- 上記が固まるまで generator / dry-run / 取得設計には進まない。進めてよいのは docs 設計（スキーマ a/b・馬ID対応方式の机上検討）まで。
- なお **uma_info を使う全履歴路線とは別に、entries 手作業コピペ運用は小さく再開できる**（§12.2／§12.7・自動取得を伴わない）。
...
- 2026-06-11: **法務判断の断定を緩和し実務方針へ整理（§12.2.2・§12.3・§12.10）**。「要許諾」「許諾が取れない限り不可寄り」「許諾確認まで保留」「規約違反リスクが高い」「個人利用を超える利用は要許諾」「商用・有料サービス内利用は再配布・二次利用に該当する公算」等の断定的・停止寄り表現を、中立・実務寄りの表現（「利用条件は未確定」「現時点では採用未決定」「運用方針が固まるまで保留」「外部サイト由来データの扱いには注意」「利用範囲の解釈には確認余地がある」「保存・表示・再利用の扱いは必要に応じて確認」）へ置換。**実務方針**＝ uma_info の自動取得・大量取得はしない／keiba.go.jp DataRoom は robots Disallow で対象外／entries は手作業コピペ運用として小さく再開できる（uma_info 路線とは別物）。**外部サイト由来テキストの扱いには注意する旨は最低限残す**。**docs-only・1ファイル。uma_info巡回/取得なし・スクレイプ/generator/dry-run/scripts/shared/AK/KI/entries-manager.astro/save-entries.mjs 変更なし**。
