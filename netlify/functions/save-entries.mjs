/**
 * Netlify Function: 出走表JSONをkeiba-data-sharedリポジトリに保存
 *
 * 機能:
 * - 出走表JSONを keiba-data-shared/{category}/entries/YYYY/MM/ に保存
...
export default async (req, context) => {
  ...
  try {
    ...
    console.log('[save-entries] リクエスト受信:', {
    ...
    // バリデーション
    if (!raceDate || !venue || !venueCode || !category || !data) {
      console.error('[save-entries] 必須フィールド不足:', { raceDate, venue, venueCode, category, hasData: !!data });
      return new Response(
        JSON.stringify({
          error: 'Missing required fields: raceDate, venue, venueCode, category, data',
          received: { raceDate, venue, venueCode, category, hasData: !!data }
        }),
        { status: 400, headers }
      );
    }
    ...
    if (!GITHUB_TOKEN) {
      console.error('[save-entries] GitHubトークンが設定されていません');
      return new Response(
        JSON.stringify({
    ...
    // ファイルパス生成: {category}/entries/YYYY/MM/YYYY-MM-DD-{venueCode}.json
    const year = raceDate.substring(0, 4);
    const month = raceDate.substring(5, 7);
    const fileName = `${raceDate}-${venueCode}.json`;
    const filePath = `${category}/entries/${year}/${month}/${fileName}`;

    console.log('[save-entries] 保存先:', filePath);
    ...
    if (getFileResponse.ok) {
      ...
      console.log('[save-entries] 既存ファイルを検出:', filePath);
    } else if (getFileResponse.status === 404) {
      console.log('[save-entries] 新規ファイルを作成:', filePath);
    } else {
    ...
    if (forceOverwrite || !existingData) {
      mergedData = data;
      console.log('[save-entries] 完全上書きモード');
    } else {
      // レース番号単位でマージ
      ...
      console.log(`[save-entries] マージ完了: ${mergedData.races.length}レース`);
    }

    // GitHub API: ファイルを作成/更新
    const putFileUrl = `https://api.github.com/repos/${GITHUB_REPO_OWNER}/${GITHUB_REPO_NAME}/contents/${filePath}`;

    const commitMessage = `📋 出走表データ追加: ${venue} ${mergedData.totalRaces}レース ${raceDate}
...
Co-Authored-By: Claude <noreply@anthropic.com>`;
    ...
    if (!putFileResponse.ok) {
      ...
      console.error('[save-entries] GitHub API エラー (put file):', {
    ...
    console.log('[save-entries] 保存成功:', result.content.sha);
    ...
    console.error('[save-entries] エラー:', error);
    return new Response(
      JSON.stringify({
