...
## 16. PR-AGE-1 実装メモ（importer 性齢抽出回帰の緩和）

```text
対象: src/pages/admin/race-data-importer.astro（extractHorse の性齢抽出のみ）

背景:
- JRA racebook の sexAge（牡3/牝4/セ5/騸5 等）取得が 2026-05-24 頃を境に急落。
  4月〜5/23 はほぼ満充足、5/24-TOK で 16/187、5/30 以降ほぼ全滅（06-06 TOK=4/171、HAN=5/161、しかも牝のみ）。
- computer は ageGender 4件のみ・horseHistories に構造化された性齢なし → 補完源が無い。
  AK/KI は shared racebook の sexAge が入れば表示できる（AK="3歳牡" / KI="牡3"、free/premium 両方）。
  → 原因は表示側ではなく admin importer の性齢抽出回帰。

修正（無回帰の追加フォールバック方式）:
- Pass1（既存・無変更）: 列内 inCol + 先頭一致 /^([牡牝セ騸])\s*(\d{1,2})/。バイト単位で従来どおり。
- Pass2（追加・Pass1 が空のときのみ）: 全角数字を半角化し、短い列内要素（length<=8）で
  /([牡牝セ騸])\s*(\d{1,2})/ を任意位置から拾う。丸数字付き行（過去走）・レースクラス語
  （未勝/新馬/勝/オープン/戦）・長い要素（父名/コメント）は除外して誤検出を抑制。
- parseSexAge は既存互換のまま（sexAge="牡3" / sex / age を生成）。
- 馬名抽出・発走時刻補完・computer 補完・assignment・score・印・買い目には触らない。

検証:
- regex/関数の単体テスト: 牡3/牝4/セ5/騸5/「牡 3」/全角「牡３」/父名結合「ディープ 牡3」を正しく抽出、
  長い父名・過去走クラス（①牝未勝/牝３勝）は誤検出せず空。Pass1 優先（先頭一致が勝つ）も確認。
- Pass1 は diff 上もロジック不変（コメント除く）。既存正常データ（4月等）には影響しない。

限界（重要）:
- 2026-05-24 以降の「破損 XML」サンプルがローカルに無いため、06-06 TOK/HAN で実際に取得率が
  改善するかは未検証。「5/24 以降 牝のみ数頭残る」パターンは shared 出力だけからは構造原因を
  断定できず、Pass2 が真因（列ズレ等）を完全には救えない可能性がある。
- マージ前に、マコさんが 06-06 TOK/HAN の元XML/貼付テキストを importer に流して
  sexAge 取得率が改善することを実機確認することを推奨。改善しない場合は inCol 列境界等の
  構造原因として別途診断する。

別件分離: featureScores / 発走時刻 / 馬名補完 / 本命対抗ロジック とは独立。騸→セ の表示正規化は
          AK/KI 両サイト同時の表示判断として別タスク。
```
...
```text
2026-06-06: PR-AGE-1 として race-data-importer.astro の性齢抽出に無回帰の Pass2 フォールバックを追加。
            Pass1（先頭一致）はそのまま、取れない場合のみ全角半角化＋任意位置一致＋誤検出ガードで救済。
            単体テストは全 PASS。ただし 5/24 以降の破損 XML が手元に無く 06-06 実機改善は未検証
            （元XML での実機確認をマージ前に推奨）。
```
