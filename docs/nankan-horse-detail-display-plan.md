...
取得元 **nankankeiba.com `uma_info` の利用条件が未確定**のため保留中
...
### 23.1 現在地
- **全履歴 `history[]` は `uma_info` の利用条件が未確定のため保留**（PR-D2 / generator / dry-run / 自動取得・大量取得はしない）。
...
## 26. Phase B-1 / entries 運用再開 runbook・可否整理（read-only・PR-E3a）(2026-06-11)

§24（AK 設計）/§25（KI 設計）の実装（PR-E1b/E1c・E2b/E2c）は **entries が最新日に供給される運用**が前提
（entries は 2026-04-07 で停止・最新日に無い＝§23.2）。本節は **entries-manager 手作業コピペ運用を再開する場合の
手順・負荷・リスク・AK/KI 反映条件**を read-only で整理し、**運用再開の価値を判断できる状態**にする。
...
- nankankeiba.com 利用規約精査（[nankan-horse-histories-detail-contract.md] §12.10）では、
  **利用範囲の解釈には確認余地がある**と整理済み。これは運用上の注意であり、entries 運用を全面停止とはしない。
- **shared 保存・AK/KI 表示・サービス内利用は、保存・表示・再利用の扱いを必要に応じて確認する**。
- **uma_info の自動取得・大量取得はしない**。entries は **手作業コピペ運用**として小さく扱う。
- 貼り付けミス / date・venue 間違い / raceNumber parse ミス → **プレビュー確認は必須**。
- **entries 保存だけでは AK/KI 反映されない**（shared 保存後も AK/KI には自動反映されない）。
  AK/KI 反映には **PR-E1b/E2b/E1c/E2c が別途必要**（後続PRで対応）。

### 26.7 AK/KI 実装へ進む条件
- **entries 運用再開を方針として決める**（運用再開は可能。まず1日1会場を小さく運用検証する）。
- **外部データ利用方針を確認する**。
...
### 26.8 実装フェーズの分岐
- **entries 運用を再開する場合**:
  - PR-E1b: AK entries import 配線。
  - PR-E2b: KI entries import 配線 ＋ included_files（`src/data/entries/**`・§25.4）。
  - PR-E1c: AK 条件付き表示。
  - PR-E2c: KI 条件付き表示。
- **entries 運用を再開しない場合**:
  - 実装は保留。現行表示維持。**全履歴路線は uma_info の利用条件・運用方針が固まるまで本格展開しない**（§12.10）。
...
### 26.10 実務方針（まとめ・2026-06-11）
南関馬詳細の供給（entries）について、現時点の実務方針を次のとおり整理する。法務判断の断定はせず、運用上の注意として扱う。

- **`uma_info` の自動取得・大量取得はしない**。
- **keiba.go.jp DataRoom は robots Disallow のため対象外**。
- **entries は手作業コピペ運用として扱う**（`entries-manager.astro` 経由）。
- **entries 運用再開は可能**。まずは **1日1会場を小さく運用検証**する。
- **shared 保存後も AK/KI には自動反映されない**（`repository_dispatch` 無し）。
- **AK/KI 反映は後続PR（PR-E1b/E2b/E1c/E2c）で対応**する。
- **外部サイト由来テキストの扱いには注意する**が、これを理由に entries 運用を全面停止とはしない。
