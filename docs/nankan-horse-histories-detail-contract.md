...
### 12.10.1 結論：可否分類＝**要許諾（許諾が取れない限り不可寄り）**

- **`uma_info` は全履歴 `history[]` の技術的な本命候補**である（§12.9.3：デビューからの全出走履歴＋通算＋条件別を 1 ページで持つ）。
- **だが利用規約精査の結果、可否分類は「要許諾」とする。** 許諾が取れるまで取得しない。
...
- 規約本文に **「別に許可を頂いている場合を除き」＋「個人的・家庭的利用に限定」** の趣旨の条項あり
  → **個人的・家庭的利用を超える複製・転載・転用・二次利用・再配布・データベース化は、許可（許諾）が必要と読むのが妥当**。
- ⚠️ **規約本文の取得に文字化け・解釈余地があり、可否の語尾を断定できない箇所がある**。
  **明示の許可とは判断しない**（保守的に「個人利用を超える利用は要許諾」と扱う）。**最終判断は南関側への確認が必要**。

### 12.10.4 本件用途が「個人利用を超える」理由
本契約の用途は次を含み、**個人的・家庭的利用を明確に超える**：
- `uma_info` の**全履歴データ取得** / **shared への保存** / **`history[]` への加工**
- **AK/KI での表示** / **有料サービス内利用** / **データベース化**
- shared（第三者リポジトリ）への保存は**再配布・二次利用と見なされる可能性**。
...
### 12.10.6 商用・有料サービス利用上のリスク
- 用途に**有料サービス内利用**・**shared 再配布**・**全履歴の DB 化**を含むため、規約の許可対象（複製・二次利用・再配布・DB化）に該当する公算が高い。
- **許諾なしに進めると規約違反リスクが高い**。
...
- → **許諾が取れない場合、`uma_info` を取得元にした全履歴 `history[]` 生成は不可**（他に現実的な全履歴取得元が無い）。

### 12.10.9 PR-D2 進行条件（更新・許諾確認まで保留）

**PR-D2 は許諾確認まで保留**。取得部分に進むには次を**すべて**満たす:
1. **nankankeiba.com `uma_info` のデータ利用許諾**。
2. **商用・有料サービス内利用の許諾**。
3. **shared 保存・加工・表示・DB化の許諾**。
4. **自動取得または取得方法に関する許諾**。
5. **取得負荷・アクセス間隔条件の確認**。
6. **馬ID→URL 対応方式の決定**（§12.9.4）。

- 上記が揃うまで generator / dry-run / 取得設計には進まない。進めてよいのは docs 設計（スキーマ a/b・馬ID対応方式の机上検討）まで。
- **許諾が取れない場合は、全履歴 `history[]` 路線そのものを見直す**（uma_info 不可 → 他に手段なし）。
...
- 2026-06-11: **PR-D1 追補3：nankankeiba.com `uma_info` 利用規約 精査結果を追記（新設 §12.10）**。規約系ページ（`/info/usage/` 本文・sitemap・izonsho・`/info/contact/`・robots.txt=404）を read-only で精査。**可否分類＝「要許諾」**（許諾が取れない限り不可寄り）と確定。著作権 `all rights reserved` 明示・権利は南関側保有・**「個人的/家庭的利用に限定」＋「別に許可を頂いている場合を除き」**＝個人利用を超える複製/転載/二次利用/再配布/DB化は許可制。本件用途（全履歴取得・shared保存・history[]加工・AK/KI表示・**有料サービス内利用**・DB化・再配布）は個人利用を超えるため要許諾。**自動取得・機械的アクセスの明示禁止は規約本文に無いが明示許可も無く、robots.txt 不在も許可扱いしない＝可否不明**。規約本文に文字化け・解釈余地があり**最終判断は南関側への確認が必要**。問い合わせは `/info/contact/`（意見向け）で正式許諾窓口として十分か不明。**keiba.go.jp HorseMarkInfo は引き続き robots Disallow で対象外／entries は全履歴を持たず補完源に留める／許諾が取れない場合 uma_info での全履歴生成は不可**。**PR-D2 は許諾確認まで保留**（進行条件＝データ利用許諾＋商用/有料利用許諾＋shared保存/加工/表示/DB化許諾＋自動取得/取得方法の許諾＋取得負荷/アクセス間隔の確認＋馬ID対応方式の決定）。**docs-only・1ファイル。uma_info巡回/取得なし・スクレイプ/generator/dry-run/scripts/shared/AK/KI/entries-manager.astro/save-entries.mjs 変更なし**。
