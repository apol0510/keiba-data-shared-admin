...
#### **問題: GitHub Actions競合エラー**
- **原因**: unmerged files検出後、indexクリアせずgit reset --hard実行
- **解決策**: 2段階リセット（git reset → git reset --hard origin/main）を全6ワークフローに適用
...
**問題:**
- 11Rのレース名が「-」（空欄）で表示される
- 正しくは「ブルートシュタインチャレンジ」が抽出されるべき

**根本原因（第1段階）:**
- HTMLに「準重賞　ＪＲＡ交流」という付帯情報が含まれる
- 抽出ロジックが付帯情報を優先的にレース名として認識

**第1回修正:**
- 付帯情報行（準重賞、ＪＲＡ交流など）を除外する正規表現を追加
- **結果**: レース名が「-」（空欄）に変化

**根本原因（第2段階）:**
- 「ブルートシュタインチャレンジ」は「賞」「杯」「特別」を含まない
- 既存の優先順位1-3では検出できなかった

**第2回修正（最終解決）:**
- **優先順位4を新設**: カタカナ主体のレース名検出
  - カタカナ5文字以上
  - 全体で5-30文字
  - 払戻金・賞金関連キーワードを除外
  - 付帯情報行を除外
...
**修正箇所:**
- results-batch.astro (lines 638-660): 優先順位4の追加
- results-manager.astro (lines 927-933): カタカナボーナススコア+50追加

**抽出優先順位（最終版）:**
1. 重賞レース名（第○回、グレード記号）
2. 地方競馬グレード（Ａ１、Ｂ２など）
3. 「特別」「賞」「杯」を含むレース名
4. **カタカナ主体レース名（5文字以上）** ← NEW
5. クラス名（３歳など）
...
- src/pages/admin/results-manager.astro

**検証結果:**
- ✅ 「ブルートシュタインチャレンジ」が正常に抽出される
- ⚠️ 一部レースで「リプレイ リプレイ」が誤検出（クラス名でフォールバック可能なため影響軽微）
