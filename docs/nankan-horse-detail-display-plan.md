...
### 34.26 R1d-scroll-v1：内側 details 開閉時のレースアコーディオン高さ再計算方針 (2026-06-13)

#### 34.26.1 背景
- R1d-impl-v2 / v2c により、本命カードの5行過去走表示、連下 / 補欠 compact 近走の通過順表示が本番反映された。
- HTML grep 上は構造破壊はなかったが、ユーザー目視で「連下以下の過去走が見えない」「下にスクロールできず次レースへ進むように見える」現象が再発した。
- 特に、本命 / 対抗の過去走を開いたまま、連下 / 補欠の過去走を開く操作で起きやすい。

#### 34.26.2 原因整理
- AK premium 南関のレース単位アコーディオンは、開いた時点の `scrollHeight` を `maxHeight` に固定する実装（`toggleNankanRace`：`content.style.maxHeight = content.scrollHeight + 'px'`）。
- 親 `.accordion-content` は `overflow:hidden` を持つ。
- 内側の過去走 `<details>` は、開閉しても親 `.accordion-content` の `maxHeight` を再計算しない（`toggle` リスナー・`scrollIntoView` ともコード上に存在しない）。
- そのため、内側 `<details>` を後から開いて高さが増えても、親の固定 `maxHeight` を超えた部分がクリップされる。
- 結果として、連下 / 補欠の過去走が見えない、またはページスクロールが次レースへ流れるように見える。
- PR #77 / #79 は根本原因ではないが、表示内容の高さを増やしたため問題を顕在化させた可能性がある（根本原因は R1d 以前からの親アコーディオン `maxHeight` 非再計算）。

#### 34.26.3 採用方針
- 採用方針は「**親アコーディオンの `maxHeight` 再計算を必須、`scrollIntoView` は補助**」とする。
- 内側 `<details>` の toggle 時に、開いている親 `.accordion-content` の `maxHeight` を現在の `scrollHeight` で再設定する。
- これにより、内側の過去走を後から開いても親の高さが追従し、クリップを防ぐ。
- 併せて、必要最小限で `scrollIntoView({ block: 'nearest' })` を使い、開いた details が視認範囲に入りやすくする。
- 同一レース内で開ける過去走を1つに制限する案は、今回は採用しない（ユーザーが本命 / 対抗 / 連下を比較しながら見たい可能性があるため）。
- 過去走の行数削減や5行カード化の巻き戻しも採用しない。

#### 34.26.4 実装境界
- 次の実装候補は AK の `astro-site/src/pages/premium-prediction/nankan.astro` **1ファイル**。
- 既存のレース単位アコーディオン制御 script に、内側 details の toggle 監視を**最小追加**する。
- 対象は **AK premium 南関のみ**。
- free / KI / JRA 側は今回は対象外。
- nankan.astro の過去走表示構造、近走データソース、通過順表示、コメント文言は変更しない。
- featureScores / 予測スコア / 評価記号 / 推奨系ロジック / dark-horse.mjs には接続しない。
- loader / inject / shared schema は変更しない。

#### 34.26.5 実装時の考え方
- 既存の race accordion が開いている時だけ、対象 `.accordion-content` の `style.maxHeight` を `scrollHeight + 'px'` に更新する。
- 内側 details の `toggle` イベントを拾う。
- toggle 対象の祖先から、該当する `.accordion-content` を特定する。
- 親 race accordion が開いている場合だけ再計算する。
- 必要なら `requestAnimationFrame` で DOM 更新後に再計算する。
- `scrollIntoView` は開いた details に対して補助的に使うが、主目的は `maxHeight` 再計算。
- SSR / 静的生成に影響しない inline client script の最小差分にする。
...
### 34.27 取消・除外・中止行の finishStatus 正規化方針 (2026-06-13)

#### 34.27.1 背景
- AK premium 南関の過去走カードで、非通常完走行（取消・除外・中止・競走中止など）が「日付・場・距離・一部メタ情報のみ」表示され、着順・人気・タイム・着差・上がり・通過順が空になり、状態（取消等）も表示されない問題が確認された。
- 直近調査の既知事例（原因追跡用。個別馬だけの特例修正にはしない）:
  - ヨシノダイセン: 2026-04-17 大井 1600
  - ヨシノダイセン: 2025-09-18 名古屋 1500 良
  - イロンデル: 2025-09-18 大井 Ｃ２(五)(六) 取消行
- shared entries の該当 race object では `finish` / `finishStatus` / `time` / `last3f` / `popularity` / `passingOrder` / `margin` が空または null。
- AK 本命 enhanced 表示側には `finishStatus` を表示する受け皿があるが、shared に `finishStatus` が入っていないため表示できない。連下 / 補欠 compact 側は現状 `finishStatus` フォールバック自体が未対応。
- `order` は近走配列内の連番であり、着順ではない（着順代替に使わない）。

#### 34.27.2 正規化方針（admin/shared を正本とする）
- 取消・除外・中止・競走中止などの非通常完走行は、**空の通常行として扱わない**。
- 着順として数値化できない行は、`finish` / `rank` に**無理に数値を入れない**。
- 代わりに `finishStatus` に `取消` / `除外` / `中止` / `競走中止` などの状態を保持する。
- `order` は近走配列内の並び順であり、**着順代替に使わない**。
- AK / KI 表示側は、shared に `finishStatus` が存在する場合**のみ** status 表示できる。shared に status が無い場合、**表示側だけで推測補完しない**。
- **AK 側だけで「取消」等を推測表示する暫定対応は禁止**（片側寄せ修正の禁止）。
- 連下 / 補欠 compact 側の `finishStatus` 表示補助は、shared に status が入ることを前提とした**後続候補**とする。

#### 34.27.3 修正順序
1. **docs 記録**（本節）。
2. **admin parser 調査 / 補強**：公式 uma_shosai の「着 / 頭数＝取消」等を `finishStatus` に確実に取り込む（当該行で None になる検出漏れの補強）。
3. **対象 entries 再取得 / 再生成**（shared 反映）。
4. **AK 必要最小補助**（連下 / 補欠 compact の `finishStatus` 表示補助など）。
- この順序を守り、表示側の場当たり修正で共通データの欠陥を隠さない。

#### 34.27.4 非対象（34.27）
- 今回は docs-only。parser 実装 / shared 再生成 / AK 修正は行わない。
- free / KI / JRA への横展開は対象外。
- shared schema 変更は行わない（`finishStatus` は既存フィールド）。
- featureScores / 予測スコア / 評価記号 / 推奨系ロジック / dark-horse.mjs には接続しない。
- 個別馬だけの特例修正にはしない（既知事例は原因追跡用）。

---

### 34.28 連下・補欠 compact の finishStatus 表示補助方針 (2026-06-13)

#### 34.28.1 背景
- §34.27 の正規化方針に沿い、admin parser 補強（PR #128・`.nk23_u-text16` から `finishStatus` を厳密一致で取得）→ shared PUT（`f1b4834`・`nankan/entries/2026/06/2026-06-12-OOI.json`・finishStatus 付き近走 25件）→ AK/KI import workflow success（AK `5115eb7` / KI `b530b77`）→ AK 本番表示確認まで完了した。
- AK premium 南関の **本命 / 対抗 / 単穴 enhanced カードには `finishStatus` 表示の受け皿があり**、本番で状態語が表示済み（§34.28.3）。
- 一方、**連下 / 補欠 compact 表示は現状 `finishStatus` フォールバック未対応**で、compact 側に状態付き近走が出た場合に状態が表示されない可能性がある。これを §34.27 の後続として記録する。

#### 34.28.2 方針（後続 AK 実装）
- compact の結果表示箇所で **`rank` / `finish` が数値化できない場合に限り**、`finishStatus` を表示候補にする。
- `order` は近走配列内の連番であり、**着順代替に使わない**。
- shared に `finishStatus` が無い場合、**AK 側で `取消` 等を推測表示しない**（admin/shared 正本・片側寄せ禁止）。
- 表示対象は**状態語そのもののみ**。無理に「着」表記へ変換しない。
- `time` / `last3f` / `popularity` / `passingOrder` などの既存 compact 表示は変更しない。
- 対象は **AK premium 南関の連下 / 補欠 compact のみ**。free / KI / JRA は今回対象外。
- featureScores / 予測スコア / 評価記号 / 推奨系ロジック / dark-horse.mjs 非接続。

#### 34.28.3 既知確認結果
- **本命 enhanced（本番確認済み）**：
  - ヨシノダイセン：2026-04-17 大井 → `能試` 表示 / 2025-09-18 名古屋 → `除外` 表示。
  - イロンデル：2025-09-18 大井 → `取消` 表示。
  - 本番 HTML 上、状態語は enhanced カード内に出現（取消2・能試7・除外1）。
- **compact（連下/補欠）**：
  - 現状、状態語付き行の**表示受け皿は未対応の可能性**（本番 HTML 上 compact 内の状態語は 0）。
  - 後続 PR で**最小追加候補**（rank/finish 非数値時の `finishStatus` フォールバック）。

#### 34.28.4 修正順序
1. docs 記録（本節）。
2. AK compact markup / helper の read-only 確認。
3. AK premium 南関 **1ファイル**最小修正（compact の `finishStatus` フォールバック）。
4. build / HTML 確認。
5. PR / 本番確認。
...
- 2026-06-13: §34.28 に **連下・補欠 compact の finishStatus 表示補助方針**を記録。admin parser 補強（PR #128）→ shared PUT（`f1b4834`・finishStatus 25件）→ AK/KI import success（AK `5115eb7` / KI `b530b77`）→ AK 本番表示確認まで完了し、**本命 enhanced では `finishStatus`（ヨシノダイセン 能試/除外・イロンデル 取消）が本番表示済み**を確認。一方 **連下/補欠 compact は finishStatus フォールバック未対応**（本番 HTML 上 compact 内の状態語 0）。後続 AK 実装方針＝**compact の結果表示で `rank`/`finish` が数値化できない場合のみ `finishStatus` を表示候補にする**（`order` は着順代替に使わない・shared に無ければ AK で推測表示しない・状態語そのものを表示し「着」変換しない・time/last3f/popularity/passingOrder 既存表示は不変）。対象は **AK premium 南関 連下/補欠 compact のみ**・free/KI/JRA 対象外・featureScores/予測スコア/評価記号/推奨系ロジック/dark-horse.mjs 非接続。修正順序＝docs 記録→AK compact markup/helper read-only 確認→AK 1ファイル最小修正→build/HTML→PR/本番確認。**docs-only・実装/shared PUT/dispatch/AK 修正なし**。
- 2026-06-13: §34.27 に **取消・除外・中止行の finishStatus 正規化方針**を記録。AK premium 南関の過去走カードで非通常完走行（取消等）が着順 / 人気 / タイム / 着差 / 上がり / 通過順とも空・状態も非表示になる問題を確認（既知事例：ヨシノダイセン 2026-04-17 大井1600・2025-09-18 名古屋1500・イロンデル 2025-09-18 大井 Ｃ２(五)(六) 取消行）。shared entries 該当 race object は `finish`/`finishStatus`/`time`/`last3f`/`popularity`/`passingOrder`/`margin` が空または null で、AK 本命 enhanced は `finishStatus` 表示の受け皿を持つが shared に値が無く表示できない（連下/補欠 compact は受け皿自体が未対応・`order` は連番で着順代替に使わない）。**正規化方針＝非通常完走行を空の通常行扱いにせず `finish`/`rank` に数値を入れず `finishStatus` に取消/除外/中止/競走中止を保持。AK/KI は shared に `finishStatus` がある場合のみ status 表示し、無い場合は表示側で推測補完しない（AK 片側の推測表示は禁止）**。修正順序＝docs 記録→admin parser 調査/補強→対象 entries 再取得/再生成→AK 最小補助。連下/補欠 compact の `finishStatus` 表示補助は status が入る前提の後続候補。**docs-only・parser 実装/shared 再生成/AK 修正なし・free/KI/JRA 横展開対象外・shared schema 不変（finishStatus は既存）・featureScores/予測スコア/評価記号/推奨系ロジック/dark-horse.mjs 非接続**。
- 2026-06-13: §34.26 に **R1d-scroll-v1** として、AK premium 南関の内側 details 開閉時に親レースアコーディオンの maxHeight が再計算されず連下 / 補欠の過去走がクリップされる問題の原因と、maxHeight 再計算＋scrollIntoView 補助による最小修正方針を記録。レース単位アコーディオン（`toggleNankanRace`）が開いた瞬間の `scrollHeight` を `maxHeight` 固定し `.accordion-content` は `overflow:hidden`、内側の過去走はネイティブ `<details>` で後から開いても親 maxHeight が再計算されず（`toggle` リスナー/`scrollIntoView` ともコード上に無し）超過分がクリップされる構造を確定。根本原因は R1d 以前からの非再計算で、PR #77（本命5行）/ PR #79（通過順）が内側高さを増やし顕在化。採用方針＝**内側 details の toggle で開いている親 `.accordion-content` の maxHeight を現在の scrollHeight に再設定（必須）＋ `scrollIntoView({block:'nearest'})` 補助**。同一レース内 details の単一化や行数削減・5行カード巻き戻しは不採用。実装候補は AK `premium-prediction/nankan.astro` の inline client script 最小追加 1ファイル、対象は **AK premium 南関のみ**、free / KI / JRA は対象外、過去走構造/データソース/通過順/コメント文言は不変・featureScores/予測スコア/評価記号/推奨系ロジック/dark-horse.mjs 非接続・loader/inject/shared schema 不変。**docs-only・AK/KI/shared/実装変更なし**。
