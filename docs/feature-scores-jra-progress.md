...
## ✅ 完了記録: JRA featureScores 96張り付き緩和（2026-06-09）

上記監査（§ 監査: featureScores 高得点集中の原因）の **修正第一候補（最小・低リスク）= 傾きを 18→14 に弱める** を実装し、shared 反映・AK/KI import・本番確認まで完了した。

### 1. 完了日
- 2026-06-09

### 2. 対象タスク名
- JRA featureScores 96張り付き緩和

### 3. 実装内容
- **JRAのみ slope 18 → 14**（`scaleFeature` 呼び出し側で category により注入）
- 南関は **slope 18 維持**（`scaleFeature` を一律変更しない）
- center 56 維持
- clamp 12〜96 維持
- null / insufficient 判定 変更なし（z射影前に確定するため slope 変更の影響を受けない）
- **engine名 `jra-v1` / `nankan-v1` 変更なし**

### 4. PR情報（keiba-data-shared-admin）
- **PR #79 merge済み**
- merge commit: `5d24e44`
- 変更ファイル: `scripts/build-feature-scores-once.mjs`（+10/-4）
  - `const SLOPE = opts.category === 'jra' ? 14 : 18;` を追加し、`scaleFeature(..., SLOPE)` で注入。

### 5. shared反映（keiba-data-shared）
- 2026-06-07 **TOK** featureScores push済み — shared commit `86c2eb1`
- 2026-06-07 **HAN** featureScores push済み — shared commit `1a4b672`
- ※既存日付の再生成は TOK/HAN のみ。それ以外の過去日は旧slopeのまま（下記「今後の注意」参照）。

### 6. AK/KI反映
- analytics-keiba import workflow_dispatch **成功** — AK run: `27171593308`
- keiba-intelligence import workflow_dispatch **成功** — KI run: `27171594858`
- AK/KI とも **shared 新版と完全一致**確認済み（engine `jra-v1`・分布一致）。

### 7. 分布改善結果
**TOK（182頭）**
| 指標 | 旧slope18 | 新slope14 |
|---|---|---|
| ≥90 | 66 | 24 |
| ≥95 | 37 | 3 |
| ==96 | 33 | 3 |
| 5項目以上≥90の馬 | 1/182 | 0/182 |
| null/insufficient | 85/1092 | 85/1092（不変） |
...
| ≥95 | 32 | 3 |
| ==96 | 28 | 3 |
| 5項目以上≥90の馬 | 1/175 | 0/175 |
| null/insufficient | 89/1050 | 89/1050（不変） |

### 8. 本番確認（read-only）
- AK/KI 本番反映**確認済み**（import commit がデプロイ済み）。
- **KI 6軸表示で張り付き緩和を確認**（多数が40〜75帯、90〜96はごく少数の真の外れ値のみ）。
- **AK は3項目派生%（安定性/能力上位性/展開利）**のため、最上位馬で高め%が残る場合があるが、これは順位ベース派生指標の**仕様**であり回帰ではない（KI/AK差別化方針どおり）。
- 表示崩れなし。
- AI指数・印・買い目・予想本文への影響なし。

### 9. 今後の注意
- **既存日付は再生成・pushしない限り旧slopeのまま**（今回新slopeで反映したのは 2026-06-07 TOK/HAN のみ）。
- 今後の **JRA featureScores 生成には新slope14が自然適用**される。
- **南関は今回対象外**（slope18 維持）。
- **engine名は変更禁止**。AK/KI の `importFeatureScores.js` が `jra-v1` / `nankan-v1` を検証しており、不一致はファイルskip＝表示消失につながる。
