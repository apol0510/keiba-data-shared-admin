...
- **自動取得は、出馬表ページ系を対象に、小規模・低負荷・停止可能な設計で進める**（保存なし dry-run first・1日1会場・§27.3）。
...
- **判断**: **robots.txt 不在を「許可」と扱わない**。著作権・利用規約・取得負荷（礼儀的アクセス間隔）を確認しながら、
  ...
  - **entries 相当の出馬表ページ系**（`/syousai/` 等）は、**「自動取得 first / 手作業 fallback」の自動取得対象**になりうる
    （[nankan-horse-detail-display-plan.md §27]・保存なし dry-run first・1日1会場）。
